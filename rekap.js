    ...
    document.getElementById('tbody-weekly').innerHTML = masters.map(m => {
        ...
        const getIcon = (idx) => {
            ...
            if (data.bukti_list && data.bukti_list.length > 0) {
                // PERBAIKAN: Escape tanda petik agar tidak merusak HTML onclick
                const stringified = JSON.stringify(data.bukti_list).replace(/'/g, "\\'");
                return `<span class="check-icon" style="cursor:pointer; text-decoration:underline;" onclick='showImagePopup(${stringified})'>✔</span>`;
            }
...
// FUNGSI POPUP (GLOBAL SCOPE)
window.showImagePopup = function(images) {
    if (!images || images.length === 0) return;

    // Hapus modal jika sudah ada sebelumnya
...
async function resetUser(id) {
    ...
    if (logs && logs.length > 0) {
        ...
        if (filesToRemove.length > 0) await _supabase.storage.from('bukti-absen').remove(filesToRemove);
    }
